...
async function boot(){
  ...
  // Merge only rigid meshes attached to each joint; never weld animated joints together.
  const jointNodes=[];hero.traverse(n=>{if(n.isGroup)jointNodes.push(n);});
  for(const node of jointNodes){const meshes=node.children.filter(n=>n.isMesh);if(meshes.length<2)continue;const rigid=new T.Group();meshes.forEach(m=>rigid.add(m));node.add(bakeStatic(rigid));}
  ...
  let started=false,paused=false,zoom=1,walk=0,elapsed=0,acc=0,last=performance.now(),fps=60,arrived=false;
  ...
  function pause(value){if(!started)return;paused=value;$('#pause').hidden=!value;input.clear();}
  ...
  $('#reset-view').onclick=()=>{hero.position.set(0,.06,11);hero.rotation.y=Math.PI;zoom=1;pause(false);};
  $('#startb').onclick=()=>{started=true;$('#welcome').hidden=true;$('#hud').hidden=false;input.clear();document.activeElement?.blur();};
  await rig.ready;
  ...
  function sim(dt){
    ...
    const m=input.read(),speed=3.4,dx=m.x*speed*dt,dz=m.z*speed*dt;
    if(world.canStand(hero.position.x+dx,hero.position.z))hero.position.x+=dx;
    if(world.canStand(hero.position.x,hero.position.z+dz))hero.position.z+=dz;
    const moving=Math.hypot(m.x,m.z)>.08;
    let angle=hero.rotation.y;
    if(moving)angle=Math.atan2(m.x,m.z);
    else if(input.pointer.active&&!input.isTouch){ray.setFromCamera(input.pointer,camera);if(ray.ray.intersectPlane(plane,aim))angle=Math.atan2(aim.x-hero.position.x,aim.z-hero.position.z);}
    ...
    const near=hero.position.z< -12&&Math.abs(hero.position.x)<4;
    if(near!==arrived){arrived=near;$('#arrival').hidden=!near;}
  }
  function frame(now){
    const realDt=(now-last)/1000;last=now;fps=T.MathUtils.lerp(fps,1/Math.max(realDt,.001),.05);const dt=Math.min(realDt,.1);elapsed+=dt;acc+=dt;
    ...
    window.__GAME__={pos:[hero.position.x,hero.position.z],fps,speed:started&&!paused?Math.hypot(input.read().x,input.read().z)*3.4:0,score:0,over:false,draws:renderer.info.render.calls,tris:renderer.info.render.triangles,started,paused,stage:1};
