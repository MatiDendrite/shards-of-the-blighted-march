...
const explode=s=>{s.m.damageShard(999);s.m.player.x=0;s.m.player.z=11;for(const e of s.m.enemies)e.stagger=100;for(let i=0;i<90;i++)s.m.update(1/60);flush(s);};
...
test('eight kills before the legacy blast finishes do not bypass new patrols',()=>{
 const old=legacy();Object.assign(old,completed());old.world.exploded=false;const p=new Progression(old),m=new Combat();p.restore(m);assert.equal(m.kills,8);assert(!m.complete);assert(m.enemies.filter(e=>e.patrol).every(e=>e.hp>0));
});
