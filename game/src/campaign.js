...
import {inTown,EXIT} from './world-map.js';
...
export class Campaign {
 ...
 get safe(){const m=this.combat;return !m.dead&&!m.player.action&&m.player.dodge<=0&&(inTown(m.player.x,m.player.z)||m.complete);}
 get nearGate(){return Math.hypot(this.combat.player.x-EXIT.x,this.combat.player.z-EXIT.z)<3;}
 observe(){
 ...
 travel(region){
  if(region===this.region||!this.unlocked(region)||!this.safe)return false;
 ...
 telemetry(){return {region:this.region,name:REGIONS[this.region].name,cleared:[...this.data.cleared],shards:this.shards,complete:this.complete,safe:this.safe,nearGate:this.nearGate,unlocked:REGIONS.map((_,i)=>this.unlocked(i))};}
}
