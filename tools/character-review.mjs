...
const label=process.argv[2]||'after',baseline=process.argv.includes('--baseline'),mageStudy=process.argv.includes('--mage-study');
...
try{
 ...
 await page.evaluate(async baseline=>{
  ...
  const mageMotion=createHeroMotion(actors[1]);
  ...
  window.captureActors=async view=>{
   ...
   mageMotion({angle:0,gait:view==='mage-walk'?1:0,walk:Math.PI*.45,dodge:view==='mage-dodge'?.20:0,dodgeAge:.14,dodgeX:1,dodgeZ:0},.4);
   if(view==='mage-face'){camera.position.set(.36,1.77,1.1);camera.lookAt(0,1.64,.02);}
 ...
 const results=[];for(const view of mageStudy?['mage','mage-side','mage-back','mage-face','mage-walk','mage-dodge']:['front','back','enemies','warrior','mage','ninja','dwarf','wolf','warden']){results.push(await page.evaluate(view=>window.captureActors(view),view));await page.screenshot({path:`${out}/${view}.png`});}
