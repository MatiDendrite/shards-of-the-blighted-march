...
for(let r=0;r<4;r++){if(r)c.travel(r);m.damageShard(999);for(let i=0;i<100;i++)m.update(1/60);for(const e of m.enemies)m.damageEnemy(e,999);p.events(m.consume(),m);c.observe();}c.travel(0);
...
try{const page=await browser.newPage(),errors=[],results=[];page.on('pageerror',e=>errors.push(e.message));page.on('response',r=>{if(r.status()>=400)errors.push(`${r.status()} ${r.url()}`);});
 ...
 for(let r=0;r<4;r++){if(r){await page.keyboard.press('KeyJ');await page.click(`[data-travel="${r}"]`);await page.waitForFunction(r=>window.__GAME__.campaign.region===r,{timeout:90000},r);}await page.evaluate(()=>new Promise(resolve=>{let n=12;function tick(){if(--n)requestAnimationFrame(tick);else resolve();}requestAnimationFrame(tick);}));await page.screenshot({path:`${out}/region-${r}.png`});const g=await page.evaluate(()=>window.__GAME__);assert(g.draws<=500);assert(g.tris<=600000);results.push({region:r,draws:g.draws,tris:g.tris});console.log(results.at(-1));}
