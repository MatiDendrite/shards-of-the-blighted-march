...
export default function generate(T) {
  ...
  for(const s of [-1,1])for(let i=0;i<14;i++){
    ...
    put(torso,new T.BoxGeometry(.014,.069,.006),brass,x,y-.015,-.20-.050*Math.cos(x*32)-t*.12-.008);
  }
  ...
  function embroidery(points){for(let n=1;n<points.length;n++){
    ...
    for(let k=0;k<steps;k++){const t=(k+.5)/steps,x=ax+(bx-ax)*t,y=ay+(by-ay)*t,fall=(.455-y)/.94,z=-.20-.05*Math.cos(x*32)-fall*.12-.010;
      const stitch=put(torso,new T.BoxGeometry(.010,Math.hypot(bx-ax,by-ay)/steps+.003,.006),thread,x,y,z);stitch.rotation.z=-Math.atan2(bx-ax,by-ay);
    }
  }}
