...
export function buildingSites(region){
 ...
 ][region].map(([x,z,scale],i)=>({kind:'house',x,z,scale,rotation:x<0?Math.PI/2:-Math.PI/2,...(i<2?{hall:i?'home':'inn'}:{})}));
 const sites=[...houses,{kind:'stall',x:6.4,z:-2,scale:.95,rotation:0},{kind:'stall',x:-7,z:17,scale:.9,rotation:Math.PI/2},{kind:'well',x:3,z:7,scale:.85,rotation:0}];
