...
export async function createTownView(scene,combat,progress,campaign,callbacks){
 ...
 function content(){if(!selected)return;const m=MAPS[campaign.region];$('#town-title').textContent=`${selected.name} · ${selected.role}`;
  ...
  $('#town-story').textContent=selected.id==='elder'?`Welcome to ${m.town}. ${advice} Return to these lanterns whenever you need to recover.`:selected.id==='merchant'?`The road is longer than it looks. A healing draught restores 65 health. I can supply you here in ${m.town}; Borin still handles forging and salvage in Hearthstead. Later regions yield stronger equipment. Compare it in your satchel before setting out.`:`Take any of the four roads out of ${m.town}. ${guide.title}${guide.target?` · ${guide.direction}, ${guide.distance} m`:''}. Your M map shows the fields, guardians and uncollected spoils. The distance is as the crow flies: follow the roads around houses and walls.`;
