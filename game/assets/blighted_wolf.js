...
export default function generate(T){
 ...
 const fur=material('fabric',0x555953),ruff=material('fabric',0x343b37),muzzle=material('fabric',0x898b7a),dark=material('fabric',0x202622),bone=material('stone',0xd6c9ac),eye=material('eyes',0xc5a35c,.45);eye.emissive.setHex(0x6f4912);
 ...
 function sweep(p,sections,m){const pos=[],idx=[],N=12;sections.forEach(([z,y,rx,ry],j)=>{for(let i=0;i<=N;i++){const a=i/N*Math.PI*2;pos.push(Math.cos(a)*rx,y+Math.sin(a)*ry,z);if(j&&i){const k=j*(N+1)+i;idx.push(k,k-1,k-N-2,k,k-N-2,k-N-1);}}});const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(pos,3));geo.setIndex(idx);geo.computeVertexNormals();return mesh(p,geo,m);}
 ...
 ell(head,dark,0,-.07,.48,.078,.054,.045);ell(head,dark,0,-.15,.31,.09,.017,.145);
 for(const s of [-1,1]){
