...
export async function loadArt(renderer){
 ...
 function finish(root){root.traverse(o=>{if(!o.isMesh)return;const m=o.material;
  ...
  m.onBeforeCompile=shader=>{
   if(ground){shader.uniforms.uMeadow={value:meadow};shader.vertexShader='attribute float meadowWeight; varying float vMeadowWeight;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvMeadowWeight = meadowWeight;');shader.fragmentShader='uniform sampler2D uMeadow; varying float vMeadowWeight;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`vec4 soilTex = texture2D(map, vMapUv); vec4 meadowTex = texture2D(uMeadow, vMapUv * .78); diffuseColor *= mix(soilTex, meadowTex, clamp(vMeadowWeight, 0.0, 1.0));`);}
