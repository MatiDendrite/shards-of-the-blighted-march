...
try{for(const mobile of [false,true]){
 ...
 await place(-1.8,13);const before=await state();await press('#smith-button');await page.waitForSelector('#inventory:not([hidden])');assert.match(await page.$eval('#bag-title',e=>e.textContent),/Borin/);
 await page.waitForFunction(()=>window.__GAME__.paused);
 const castPause=await page.evaluate(async()=>{
  const {scene,model}=window.serviceFixture,roles=['smith','merchant','elder','guide'];const snapshot=()=>roles.map(role=>{const root=scene.getObjectByName(`npc-${role}`),parts=[];if(!root)throw Error(`Missing ${role}`);root.traverse(o=>{if(o.isGroup)parts.push([...o.position.toArray(),...o.rotation.toArray(),...o.scale.toArray()]);});return{role,parts};});
  const before=snapshot(),time=model.time;for(let i=0;i<4;i++)await new Promise(requestAnimationFrame);return{before,after:snapshot(),time,afterTime:model.time};
 });assert.deepEqual(castPause.before,castPause.after);assert.equal(castPause.time,castPause.afterTime);
 await press('[data-item-action="upgrade"]');await page.waitForFunction(gold=>window.__GAME__.rpg.gold===gold,{},before.rpg.gold-40);assert.equal((await state()).rpg.items.find(i=>i.id===1).upgrade,1);await press('#bag-close');await page.screenshot({path:`${out}/${mobile?'mobile':'desktop'}-blacksmith.png`});
 ...
 await press('#journal-button');await page.keyboard.press('KeyE');assert.equal((await state()).campaign.region,0);await press('#journal-close');const departure=await state();await press('#gate-button');await page.waitForFunction(()=>window.__GAME__.campaign.region===1);assert.equal(await page.evaluate(()=>window.serviceFixture.scene.getObjectByName('npc-smith').visible),false);
 ...
 report.runs.push({device:mobile?'phone':'desktop',viewport,merchant:'sale, cancellation, protected loadout, potion, reload PASS',smith:'paid +1 upgrade PASS',cast:'four distinct role models, paused poses stable, smith hidden outside Hearthstead PASS',portals:'world-only forward and return, paused E blocked, loot preserved PASS',gateDistanceRange:[Math.min(...distances),Math.max(...distances)],sampledDraws:returned.draws,sampledTriangles:returned.tris});console.log(report.runs.at(-1));await context.close();
