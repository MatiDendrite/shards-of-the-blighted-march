...
const out='_artifacts/journey-camera';await fs.mkdir(out,{recursive:true});
const main=(await fs.readFile('game/src/main.js','utf8')).replace('await rig.ready;',`window.journeyFixture={model,progress,campaign,world,hero,orbit,input,combatView,movement};await rig.ready;`);
...
try{for(const mobile of [false,true]){
 ...
 if(!mobile){
  ...
  const start=await state();await page.mouse.move(710,350);await page.mouse.down({button:'right'});await page.mouse.move(500,350,{steps:8});await page.mouse.up({button:'right'});await settle();const during=await state();assert(delta(during.yaw,start.yaw)>1);assert(Math.abs(during.x-start.x)<.01);assert(during.z<start.z);assert(delta(during.angle,Math.PI)<.01);
  await page.keyboard.up('KeyW');await page.waitForFunction(()=>window.__GAME__.speed===0);const stopped=await state();await page.keyboard.down('KeyW');await page.waitForFunction(p=>Math.hypot(window.__GAME__.pos[0]-p.x,window.__GAME__.pos[1]-p.z)>.4,{},stopped);await page.keyboard.up('KeyW');const moved=await state(),dx=moved.x-stopped.x,dz=moved.z-stopped.z;assert((dx*-Math.sin(stopped.yaw)+dz*-Math.cos(stopped.yaw))/Math.hypot(dx,dz)>.98);
 }else{
  ...
  await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[move,cam]});for(let i=1;i<=8;i++)await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[move,{...cam,x:cam.x+i*12}]});await page.waitForFunction(()=>Math.abs(window.__GAME__.camera.yaw)>.8);const during=await state();assert(Math.abs(during.x-start.x)<.01);assert(during.z<start.z);assert(delta(during.angle,Math.PI)<.01);
 ...
 console.log(`${device}: independent camera, held movement and clean release PASS`);
