...
export function createClassEffects(scene){
 ...
 function update(model){
   ...
   for(const s of model.projectiles){let mesh=shots.get(s.id);if(!mesh){mesh=new T.Mesh(s.kind==='venom'?knife:sphere,materials[s.kind]);if(s.kind==='firebolt')mesh.scale.set(.21,.21,.37);if(s.kind==='arcane')mesh.scale.set(.12,.12,.32);root.add(mesh);shots.set(s.id,mesh);}mesh.position.set(s.x,heightAt(s.x,s.z)+.85,s.z);mesh.rotation.set(s.kind==='venom'?Math.PI/2:0,s.angle,0);if(s.kind==='venom')mesh.quaternion.setFromUnitVectors(up,direction.set(Math.sin(s.angle),0,Math.cos(s.angle)));}
