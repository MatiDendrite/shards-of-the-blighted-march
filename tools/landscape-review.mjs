...
// Forward travel/movement use real input; the final reload uses a saved fixture.
...
try{
  ...
  for(let region=0;region<4;region++){
    if(region){await page.click('#gate-button');await page.waitForFunction(r=>window.__GAME__.campaign.region===r,{},region);}
    ...
    await page.keyboard.down('KeyW');await page.waitForFunction(()=>window.__GAME__.campaign.nearGate||window.__GAME__.campaign.region===3&&window.__GAME__.pos[1]<-54,{timeout:300000,polling:'raf'});await page.keyboard.up('KeyW');
  ...
  // Region restoration is fixture-based; scene reuse has a separate cache test.
  await loadRegionFixture(page,fixture,1);
