...
export function cameraTemplate(kind,root){
 ...
 const compound=['house','gate','stall','well'].includes(kind),parts=[];
 ...
 return{bounds,parts:compound?parts:null};
}
export function placeCameraObstacle(template,prop){
 const matrix=new T.Matrix4().compose(new T.Vector3(prop.x,0,prop.z),new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),prop.rotation),new T.Vector3(prop.sx,prop.sy,prop.sz));
 const bounds=template.bounds.clone().applyMatrix4(matrix);
 return{min:bounds.min,max:bounds.max,parts:template.parts?.map(b=>b.clone().applyMatrix4(matrix))||null};
}
