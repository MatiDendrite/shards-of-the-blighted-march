...
try{
 ...
 const p=await page.$eval('#stick',e=>{const r=e.getBoundingClientRect();return{x:r.x+r.width/2,y:r.y+r.height/2};});await page.touchscreen.touchStart(p.x,p.y);await page.touchscreen.touchMove(p.x,p.y-35);await wait(()=>window.__GAME__.pos[1]<5.5);await page.touchscreen.touchEnd();
 ...
 await tap('[data-item-action="equip"][data-id="2"]');await wait(()=>window.__GAME__.weapon==='axe');await tap('#bag-close');
