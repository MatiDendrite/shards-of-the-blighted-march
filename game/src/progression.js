...
export function validSave(s){
 ...
 if(!Array.isArray(s.claimed)||s.claimed.length>8||!s.claimed.every(i=>integer(i,1,8))||new Set(s.claimed).size!==s.claimed.length)return false;
 if(!Array.isArray(s.drops)||s.drops.length>9||!s.drops.every(d=>d&&typeof d.id==='string'&&d.id.length<30&&Number.isFinite(d.x)&&Math.abs(d.x)<40&&Number.isFinite(d.z)&&Math.abs(d.z)<40&&integer(d.gold,0,200)&&integer(d.ore,0,10)&&(!d.item||validItem(d.item))))return false;
 const allIds=[...s.items.map(i=>i.id),...s.drops.flatMap(d=>d.item?[d.item.id]:[])];
 if(new Set(allIds).size!==allIds.length||allIds.some(id=>id>=s.serial)||new Set(s.drops.map(d=>d.id)).size!==s.drops.length)return false;
 const w=s.world;return !!w&&Number.isFinite(w.shardHp)&&w.shardHp>=0&&w.shardHp<=250&&typeof w.exploded==='boolean'&&(!w.exploded||w.shardHp===0)&&typeof s.shardReward==='boolean';
}
...
export class Progression{
 constructor(saved=null){this.data=saved?structuredClone(saved):{version:1,level:1,xp:0,gold:60,ore:2,potions:3,serial:5,run:1,deaths:0,weapon:'sword',items:KINDS.map((kind,n)=>({id:n+1,kind,rarity:'common',power:kind==='armor'?2:0,upgrade:0})),loadout:{sword:1,axe:2,spear:3,armor:4},claimed:[],drops:[],shardReward:false,world:{shardHp:250,exploded:false}};this.revision=0;this.messages=[];this.potionCD=0;}
 ...
 restore(combat){const d=this.data,w=d.world;combat.player.weapon=d.weapon;if(w.shardHp<250)combat.damageShard(250-w.shardHp);if(w.exploded){combat.shard.exploded=true;combat.shard.blast=0;}for(const e of combat.enemies)if(d.claimed.includes(e.id)){e.hp=0;e.phase='dead';}combat.kills=d.claimed.length;combat.consume();this.sync(combat,true);}
 snapshot(combat){this.data.world={shardHp:combat.shard.hp,exploded:combat.shard.exploded};this.data.weapon=combat.player.weapon;return structuredClone(this.data);}
 ...
 makeItem(kind,rarity){const d=this.data;return{id:d.serial++,kind,rarity,power:(kind==='armor'?2:0)+RARITIES.indexOf(rarity)*3,upgrade:0};}
 events(events,combat){const d=this.data;for(const e of events){
  if(e.type==='kill'&&!d.claimed.includes(e.id)){const enemy=combat.enemies.find(n=>n.id===e.id);d.claimed.push(e.id);this.experience(enemy.kind==='wolf'?25:45);const seed=e.id+d.run;const item=enemy.kind==='raider'?this.makeItem(KINDS[((e.id*17+d.run*23)%11)%4],seed%3===0?'rare':'uncommon'):null;d.drops.push({id:`${d.run}-${e.id}`,x:enemy.x,z:enemy.z,gold:enemy.kind==='wolf'?12:24,ore:enemy.kind==='raider'?1:0,item});this.touch(`+${enemy.kind==='wolf'?25:45} XP · loot dropped`);}
  if(e.type==='explosion'&&!d.shardReward){d.shardReward=true;this.experience(80);d.drops.push({id:`${d.run}-shard`,x:combat.shard.x,z:combat.shard.z,gold:60,ore:3,item:this.makeItem(KINDS[d.run%4],'rare')});this.touch('Shard cleansed · rare equipment dropped');}
 ...
 nearSmith(player){return ! (player.hp<=0)&&Math.hypot(player.x-SMITH.x,player.z-SMITH.z)<3;}
 ...
 nextRun(combat){this.data.run++;this.data.claimed=[];this.data.drops=[];this.data.shardReward=false;this.data.world={shardHp:250,exploded:false};combat.reset();combat.player.weapon=this.data.weapon;this.potionCD=0;this.sync(combat,true);this.touch('A new hunt · your equipment and level are kept');}
