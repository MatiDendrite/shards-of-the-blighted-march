...
export function createCombatEffects(scene,heightAt=()=>0){
 ...
 function update(dt){if(!mesh.visible)return;let alive=0;
  particles.forEach((p,i)=>{if(p.life>0){p.age+=dt;if(p.age>=p.life)p.life=0;else{alive++;p.vy-=dt*5.8;p.x+=p.vx*dt;p.y+=p.vy*dt;p.z+=p.vz*dt;}}
   const scale=p.life>0?1-p.age/p.life:0;matrix.position.set(p.x||0,Math.max(heightAt(p.x||0,p.z||0)+.13,p.y||0),p.z||0);matrix.rotation.set(i*.91,i*.7,0);matrix.scale.set(scale*.65,scale*(p.heavy?3.8:2.3),scale*.65);matrix.updateMatrix();mesh.setMatrixAt(i,matrix.matrix);
