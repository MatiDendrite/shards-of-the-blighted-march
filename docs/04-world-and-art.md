...
A dressing pass added ten procedural props — supply carts, merchant stock piles, woodpiles with lean-to roofs, round hay bales, travellers' camps with campfires, ruined colonnades, berry brambles, fallen trunks with toadstools, crossroads waymarkers and split-rail fences. Each follows the procedural object contract used by 404 generation pipelines: deterministic code with no randomness, named parts, standard PBR values per material, instancing for repeated parts such as spokes, logs, flutes, leaves and berries, and `Vector2` profiles for lathe shapes. Every prop passes that pipeline's JavaScript checker, normalised to its unit cube, and the recipe's four-side verification in metres.

A deterministic layout places roughly 80 to 120 props per region: stock and firewood beside houses, carts in side streets, waymarkers at town exits, camps in the meadows, ruins near the old sanctuaries, fences in the farmland and brambles and trunks across the wilds. Placement keeps roads, settlement lanes, water, bridges, portals, doorways, townsfolk and every guardian's clearing open, and larger props receive colliders. The layout is computed with the rest of the region, while meshes load and bake into culled 10 m sectors only after the player starts, a few milliseconds per frame. A 4 m spatial index over each collider list keeps scattering and per-frame movement checks fast despite the extra obstacles.

## Grounding, clear sightlines and sound

Props previously took the ground height at their centre, so long or wide pieces hung over the downhill side of a slope. Every prop now settles to the lowest ground under its footprint, stalls and gates included, and dressing props only choose fairly level sites; houses and wells keep their graded settlement pads. Tree crowns no longer hide the hero: leaf and needle shaders stipple out fragments inside a narrow cylinder between the hero's head and the camera, while shadows stay intact, and trunk camera obstacles stop below the crown.

Sound is synthesised with Web Audio at play time. Each class ability, projectile impact and new enemy attack has its own voice, fishing casts, bites and catches splash, and filtered-noise beds for running water, wind and a crackling hearth follow the hero. No audio files are downloaded.

## Landmarks, discoveries, life and events

Each region gained a set piece: a working windmill among wheat fields in Hearthstead, an ancient oak ringed by glowing mushrooms in Thornwood, a beached wreck and an offshore lighthouse with a sweeping beam on the Causeway, and a walkable cathedral ruin in the Court. They sit on surveyed level ground with their own colliders and scenery-free floors, and their moving parts — sails, beam and mushroom glow — stay live after the rest is baked.
