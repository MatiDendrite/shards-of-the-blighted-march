...
try{const page=await browser.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.setViewport({width:1100,height:700});await page.setRequestInterception(true);page.on('request',r=>r.url().endsWith('/src/main.js')?r.respond({status:200,contentType:'text/javascript',body:''}):r.continue());await page.goto('http://localhost:4173/preview/');
 const report=await page.evaluate(async()=>{
  const T=await import('three'),{loadActorAsset}=await import('./src/actor-surfaces.js'),{Combat,enemyAttack}=await import('./src/combat-model.js'),{createCombatView,mergeJoints}=await import('./src/combat-view.js');
  const scene=new T.Scene(),hero=await loadActorAsset(new URL('./assets/wanderer.js',location.href).href,{keepHierarchy:true,height:1.85});mergeJoints(hero);scene.add(hero);hero.position.set(2.5,.06,0);hero.rotation.y=Math.PI;
  ...
  hero.rotation.x=-.35;hero.userData.joints.rightArm.rotation.set(-2,.7,0);model.spawn('raider',0,-3);const wolf=model.spawn('wolf',4.5,0);
  const result=[];for(const phase of ['idle','windup','recovery','idle']){for(const e of model.enemies){e.phase=phase;e.timer=phase==='recovery'?enemyAttack(e).recovery-.1:phase==='idle'?0:.5;}view.update(1/60,camera);scene.updateMatrixWorld(true);const invalid=[];scene.traverse(o=>{if(o.isMesh&&!o.matrixWorld.elements.every(Number.isFinite))invalid.push({name:o.name,parent:o.parent?.name});});const heads=scene.children.filter(r=>r!==hero&&r.getObjectByName('leftLeg')).map(r=>r.getObjectByName('head').getWorldPosition(new T.Vector3()).y),wolfJaw=scene.getObjectByName(`enemy-${wolf.id}`).getObjectByName('jaw').rotation.x;view.update(0,camera);const pausedJaw=scene.getObjectByName(`enemy-${wolf.id}`).getObjectByName('jaw').rotation.x;result.push({phase,invalid,heads,wolfJaw,pausedJaw});}
  model.enemies.forEach(e=>e.phase='idle');view.update(0,camera);renderer.render(scene,camera);return result;
 });await page.screenshot({path:'_artifacts/hud-loot/actors.png'});console.log(JSON.stringify(report,null,2));assert.deepEqual(errors,[]);assert(report.every(r=>r.invalid.length===0),'all enemy body parts must have finite world transforms in every phase');assert(report.every(r=>r.heads.length===3&&r.heads.every(y=>y>1.4)),'all raider/boss heads, including a late reinforcement, must remain above ground');assert(report.every(r=>r.phase==='idle'?r.wolfJaw===0:r.wolfJaw>0),'wolf jaw opens only during its actual attack, then closes');assert(report.every(r=>r.wolfJaw===r.pausedJaw),'paused secondary motion must not drift');
