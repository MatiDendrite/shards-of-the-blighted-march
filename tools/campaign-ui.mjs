...
// Explicit save fixtures exercise migration/region restoration and mobile journal UI.
...
for(let i=0;i<3;i++){m.damageShard(999);for(let n=0;n<100;n++)m.update(1/60);for(const e of m.enemies)m.damageEnemy(e,999);p.events(m.consume(),m);c.observe();travelFixture(c,i+1);fixtures.push(p.snapshot(m));}
...
try{for(let region=0;region<4;region++){
 ...
 assert.equal(await page.$$eval('[data-travel]',es=>es.length),0);await page.tap('#journal-close');
 if(region>0){const gold=await page.evaluate(()=>window.__GAME__.rpg.gold);await page.reload();await page.waitForFunction(()=>window.__READY__,{timeout:90000});await page.tap('#startb');assert.equal(await page.evaluate(()=>window.__GAME__.campaign.region),region);assert.equal(await page.evaluate(()=>window.__GAME__.rpg.gold),gold);}
