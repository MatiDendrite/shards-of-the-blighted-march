...
try{
 ...
 assert.equal(await page.evaluate(()=>window.__GAME__.alive),20);assert.match(await page.$eval('#objective-detail',e=>e.textContent),/0 \/ 24 guardians/);
 if(mobile)assert(await page.$eval('#objective-detail',e=>{const r=e.getBoundingClientRect(),map=document.querySelector('#navigation-map').getBoundingClientRect();return r.height>0&&r.bottom<map.top;}),'touch kill counter must stay visible above the minimap');
  ...
  if(!mobile){
   ...
   await page.waitForFunction(()=>window.__GAME__.kills===8);await page.keyboard.press('KeyJ');await page.waitForSelector('#journal:not([hidden])');assert.match(await page.$eval('[data-region="1"]',e=>e.textContent),/SEALED/);assert.match(await page.$eval('.quest-card.current',e=>e.textContent),/Guardians 8 \/ 24/);await page.screenshot({path:`${out}/sealed-journal.png`});
   await page.evaluate(()=>{const f=window.fixture;for(const e of f.model.enemies)if(e.id!==24)f.model.damageEnemy(e,999);f.progress.events(f.model.consume(),f.model);f.campaign.observe();localStorage.setItem('shards.journey.v1',JSON.stringify(f.progress.snapshot(f.model)));});
   await page.reload();await page.waitForFunction(()=>window.__READY__);await page.click('#startb');await page.waitForFunction(()=>window.__GAME__.kills===23);const restored=await page.evaluate(()=>window.__GAME__);assert(!restored.complete);assert.deepEqual(restored.enemies.map(e=>e.id),[24]);
   await page.evaluate(()=>{const f=window.fixture;f.setPaused(true);f.model.damageEnemy(f.model.enemies.find(e=>e.id===24),999);f.progress.events(f.model.consume(),f.model);f.campaign.observe();});await page.waitForFunction(()=>window.__GAME__.campaign.cleared[0]);await page.keyboard.press('KeyJ');await page.waitForSelector('#journal:not([hidden])');assert.match(await page.$eval('[data-region="1"]',e=>e.textContent),/ROAD OPEN/);assert.equal(await page.$eval('[data-travel]',es=>es.length),0);await page.click('#journal-close');await page.evaluate(()=>{window.fixture.model.player.x=0;window.fixture.model.player.z=-56;});await page.waitForSelector('#gate-button:not([hidden])');await page.click('#gate-button');await page.waitForFunction(()=>window.__GAME__.campaign.region===1);assert.equal(await page.evaluate(()=>window.__GAME__.alive),20);report.gates='8/24 locked; 23/24 locked after reload; 24/24 opens the physical portal';console.log('Gate and last-guardian reload PASS');
  }
  await context.close();
 }
