...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const reports=[];
  for(const name of ['ground','stone','paving','tile','needles','leaves','timber','plaster','meadow']){const a=read(optimized,name),b=read(fallback,name);let total=0,max=0;for(let i=0;i<a.length;i++){const d=Math.abs(a[i]-b[i]);total+=d;max=Math.max(max,d);}reports.push({name,meanDifference:total/a.length,maxDifference:max});}
