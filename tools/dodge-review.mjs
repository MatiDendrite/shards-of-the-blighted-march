...
try{
 ...
 if(mobile)assert(await page.$eval('#objective-detail',e=>{const r=e.getBoundingClientRect(),map=document.querySelector('#navigation-map').getBoundingClientRect();return r.height>0&&r.bottom<map.top;}),'touch kill counter must stay visible above the minimap');
  ...
  if(!mobile){
   ...
   await page.waitForFunction(()=>window.__GAME__.kills===8);await page.keyboard.press('KeyJ');await page.waitForSelector('#journal:not([hidden])');assert.match(await page.$eval('[data-region="1"]',e=>e.textContent),/SEALED/);assert.match(await page.$eval('.quest-card.current',e=>e.textContent),/Guardians 8 \/ 16/);await page.screenshot({path:`${out}/sealed-journal.png`});
