...
const label=process.argv[2]||'after',baseline=process.argv.includes('--baseline');
...
try{
 ...
 await page.evaluate(async baseline=>{
  const T=await import('three'),{ASSET}=await import('./lib/assetlib.js'),{mergeJoints}=await import('./src/combat-view.js'),{createRig}=await import('./lib/rig.js');
  ...
  window.captureActors=async view=>{
   ...
   const detail=['warrior','mage','ninja','dwarf','wolf','warden'].indexOf(view),enemy=view==='enemies',list=detail>=0?[actors[detail]]:enemy?[actors[4],actors[6],actors[5]]:actors.slice(0,4),names=detail>=0?[['Warrior','Mage','Ninja','Dwarf','Blighted Wolf','Fallen Warden'][detail]]:enemy?['Blighted Wolf','Hollow Raider','Fallen Warden']:['Warrior','Mage','Ninja','Dwarf'];
   list.forEach((root,i)=>{root.position.set((i-(list.length-1)/2)*(enemy?2.6:1.85),.005,0);root.rotation.set(0,view==='back'?Math.PI:0,0);scene.add(root);});
   if(detail>=0){const tall=detail===5;camera.position.set(2.1,tall?2.9:2.3,tall?6.3:4.7);camera.lookAt(0,tall?1.25:.87,0);}
 ...
 const results=[];for(const view of ['front','back','enemies','warrior','mage','ninja','dwarf','wolf','warden']){results.push(await page.evaluate(view=>window.captureActors(view),view));await page.screenshot({path:`${out}/${view}.png`});}
