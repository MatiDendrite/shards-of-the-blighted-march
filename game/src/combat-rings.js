...
export function createCombatRings(scene,prepare=()=>{}){
 ...
 function take(geo,color,x,y,z,angle,life,size,fixed){let effect=pool.find(e=>!e.mesh.visible);
  if(!effect&&pool.length<capacity){const mesh=new T.Mesh(geo,new T.MeshBasicMaterial({color,side:T.DoubleSide,transparent:true,opacity:.6,depthWrite:false,forceSinglePass:true}));scene.add(mesh);prepare(mesh);effect={mesh};pool.push(effect);}
  if(!effect)effect=pool.reduce((old,e)=>e.serial<old.serial?e:old);
  const mesh=effect.mesh;mesh.geometry=geo;mesh.material.color.set(color);mesh.material.opacity=.6;mesh.position.set(x,y,z);mesh.rotation.set(-Math.PI/2,0,angle);mesh.scale.setScalar(1);mesh.visible=true;Object.assign(effect,{age:0,life,size,fixed,serial:++serial});
 }
 ...
 function update(dt){for(const e of pool){if(!e.mesh.visible)continue;e.age+=dt;if(e.age>=e.life){e.mesh.visible=false;continue;}if(!e.fixed)e.mesh.scale.setScalar(e.size*(.3+e.age/e.life*.7));e.mesh.material.opacity=Math.max(0,1-e.age/e.life)*.6;}}
 function clear(){for(const e of pool)e.mesh.visible=false;}
 function dispose(){for(const e of pool){e.mesh.removeFromParent();e.mesh.material.dispose();}geometries.forEach(g=>g.dispose());pool.length=0;geometries.clear();}
