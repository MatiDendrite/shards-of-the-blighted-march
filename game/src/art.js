...
export async function loadArt(renderer){
 ...
 function finish(root){root.traverse(o=>{if(!o.isMesh)return;const m=o.material;
  ...
  m.onBeforeCompile=shader=>{
   if(ground){shader.uniforms.uMeadow={value:meadow};shader.vertexShader='attribute float meadowWeight; attribute float sandWeight; varying float vSandWeight; varying float vMeadowWeight;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvMeadowWeight = meadowWeight; vSandWeight=sandWeight;');shader.fragmentShader='uniform sampler2D uMeadow; varying float vSandWeight; varying float vMeadowWeight;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`vec4 soilTex = texture2D(map, vMapUv); vec4 meadowTex = texture2D(uMeadow, vMapUv * .78); float grain=fract(sin(dot(vMapUv*700.0,vec2(12.9898,78.233)))*43758.5453); float ripples=sin(vMapUv.y*65.0+sin(vMapUv.x*23.0))*0.015; vec4 sandTex=vec4(vec3(.78,.68,.51)*(.95+grain*.08+ripples),1.0); diffuseColor *= mix(mix(soilTex, meadowTex, clamp(vMeadowWeight, 0.0, 1.0)),sandTex,clamp(vSandWeight,0.0,1.0));`);}
   if(wind){shader.uniforms.uLandscapeTime=time;shader.vertexShader='uniform float uLandscapeTime;\n'+shader.vertexShader;const amount=m.name==='leaves'?'.045':m.name==='banner'?'.028':'.08';shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>\nfloat breeze = sin(uLandscapeTime * 1.5 + position.x * .8 + position.z * .55); transformed.x += breeze * ${amount} * min(1.0, max(0.0, position.y)); transformed.z += cos(uLandscapeTime + position.x * .5) * ${amount} * .3 * min(1.0, max(0.0, position.y));`);}
   if(cover){shader.uniforms.uGrassFocus=focus;shader.vertexShader='uniform vec2 uGrassFocus;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','transformed.y *= 1.0-smoothstep(24.0,34.0,distance(position.xz,uGrassFocus));\n#include <project_vertex>');}
  };m.customProgramCacheKey=()=>`landscape-v1-${m.name}`;m.needsUpdate=true;
 });}
