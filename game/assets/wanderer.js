...
export default function generate(T) {
  ...
  const sphere=(p,m,x,y,z,w,h,d)=>{const o=put(p,new T.SphereGeometry(1,10,7),m,x,y,z);o.scale.set(w,h,d);return o;};
  ...
  const capeGeo=new T.PlaneGeometry(.58,.83,6,8), positions=capeGeo.attributes.position;
  for(let i=0;i<positions.count;i++){const y=positions.getY(i);positions.setZ(i,-.055*Math.cos(positions.getX(i)*28)-(0.415-y)*.10);}
  capeGeo.computeVertexNormals();const cape=put(torso,capeGeo,cloth.clone(),0,-.015,-.20);cape.material.side=T.DoubleSide;cape.name='cape';
  ...
  put(head,new T.LatheGeometry(pts,12),iron,0,0,0);
  ...
  for(const s of [-1,1]){
    ...
    box(shin,leather,0,-.435,.065,.19,.17,.32);
